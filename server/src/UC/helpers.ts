...
import { IWithReference, ISymbol, UCSymbol, UCStructSymbol, ObjectsTable, UCTypeFlags, UCMethodSymbol, UCFieldSymbol, UCClassSymbol } from './Symbols';
...
export async function getFullCompletionItem(item: CompletionItem): Promise<CompletionItem> {
	// item.data could in theory be representing an object hash, which we can use to retrieve its symbol and fill the item's properties.
	if (item.data) {
